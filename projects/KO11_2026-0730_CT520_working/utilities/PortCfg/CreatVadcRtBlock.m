...
% ACI_xxx simulink signals must be created previously.
% Creat VADC slx model
% name the signal
%     name = VADCST.Elements(i).Name;
...
%%
% creat a slx file
modelName = [mdlNameCore0 '/SAQ/vadcRt'];
...
outputSignalNumber = length(VADCST.Elements);
...
for i = 1:outputSignalNumber
    ...
    add_line(modelName,['rtBlock',num2str(i),'/1'],['GetAdc',num2str(i),'/1']);
    add_line(modelName,['GetAdc',num2str(i),'/1'],[outBusName,'/',num2str(i)]);
    % name the signal
    name = VADCST.Elements(i).Name;
    signalName = ['SAQ_' name];
    ph = get_param(adcBlockName,'PortHandles');
    lh = get_param(ph.Outport,'Line');
    set_param(lh,'Name',signalName);
    set_param(ph.Outport,'MustResolveToSignalObject','on');
    % check the enumeration channel
    set_param([adcBlockName '/Enumerated Constant'],'OutDataTypeStr',['Enum: EVADC']);
    set_param([adcBlockName '/Enumerated Constant'],'Value',['EVADC.' name]);
    set_param(adcBlockName,'LinkStatus','inactive');  	% disable the link
    if         strcmp(name,'SINH')       || ...          % comment out SINH
               strcmp(name,'SINL')       || ...          % comment out SINL
               strcmp(name,'COSH')       || ...          % comment out COSH
               strcmp(name,'COSL')       || ...          % comment out COSL
               strcmp(name,'HVUP')       || ...          % comment out HVUP
               strcmp(name,'HVUN')       		         % comment out HVUN    
               
               set_param(rtBlockName ,'Commented','on');     % comment out rt 
               set_param(adcBlockName ,'Commented','on');    % comment out adc
...
% 
clear blockName modelName signalName rtBlockName adcBlockName
