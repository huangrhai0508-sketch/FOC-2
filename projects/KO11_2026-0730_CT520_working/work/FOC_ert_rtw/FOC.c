...
void FOC_step0(void)                   /* Sample time: [0.0001s, 0.0s] */
{
   ...
   *  Product: '<S2>/Product1'
   */
  rtb_Add5 = (uint16_T)(((int16_T)floorf(f32_P_AngleCompCur * ((real32_T)
    rtb_CastToSingle1))) + rtb_angleElec);

  /* Sum: '<S2>/Add6' incorporates:
   *  Constant: '<S2>/Constant1'
   *  Product: '<S2>/Product3'
   */
  rtb_angleElec = (uint16_T)(((uint32_T)((uint16_T)((int16_T)floorf(((real32_T)
    rtb_CastToSingle1) * f32_P_AngleCompVol)))) + rtb_Add5);
   ...
   *  Sum: '<S38>/Sum1'
   */
  rtb_Sqrt_l = look1_iflf_pbinlca(ISP_iQaFild, (const real32_T *)
    &cPsiIsqAdptMot_TT.bp1[0], (const real32_T *)&cPsiIsqAdptMot_TT.tab[0],
    &pmsmfoc_rtDWork.m_bpIndex_c, 120U);
  ...
  switch (rtAction) {
   case 0:
     ...
     *  Product: '<S22>/Divide'
     */
    rtb_Add1_l = (1.0F / fmaxf(62.831852F, fabsf(((u16_P_MotorPair / 60.0F) *
     ...
     *  Product: '<S29>/Divide'
     */
    rtb_Sqrt_l = -fabsf(g_msgCore0to2_maxCur * fminf
    ...
    if (g_msgCore0to2_CalibEn) {
       ...
       *  Abs: '<S29>/Abs'
       */
      rtb_Sqrt = look2_iflf_pbinlca(rtb_Sqrt, 0.0F, (const real32_T *)
        &MTPA_T.bp1[0], (const real32_T *)&MTPA_T.bp2[0], (const real32_T *)
        &MTPA_T.tab[0], pmsmfoc_rtDWork.m_bpIndex_j, rtCP_MTPA_T_maxIndex, 21U);
    }
     ...
     *  MinMax: '<S29>/MinMax1'
     */
    rtb_Gain3_f = fmaxf(rtb_Sqrt_l, fminf(PMC_FFW_opt, PMC_MTPA_opt));
     ...
     *  MinMax: '<S20>/Min2'
     */
    rtb_Gain2_d = fmaxf(PMC_MTPA_opt, rtb_Sqrt_l) - rtb_Gain3_f;
     ...
     *  MinMax: '<S1>/Min'
     */
    rtb_Subtract_a = fminf(f32_P_Modulation, pmsmfoc_rtU.f32_ModulationReq) -
      usage;
    ...
    if ((rtb_Sqrt_l - (rtb_Gain2_d * rtb_Gain2_d)) <= 0.0F) {
       ...
       *  Abs: '<S32>/Abs'
       */
      pmsmfoc_rtDWork.Sqrt = sqrtf(fabsf(rtb_Sqrt_l));
   ...
   *  Sum: '<S13>/Add'
   */
  rtb_Gain2_d = sqrtf((PMC_IsqDes * PMC_IsqDes) + (PMC_IsdDes * PMC_IsdDes)) *
    0.707106769F;
   ...
   */
  rtb_Gain2_d = (sqrtf(fmaxf((PMC_isqFlt * PMC_isqFlt) + (PMC_isdFlt *
    PMC_isdFlt), 0.0F)) * 0.05F) + (pmsmfoc_rtDWork.UnitDelay1_DSTATE_e * 0.95F);
  ...
  if (pmsmfoc_rtU.Drive) {
    ...
    temp2 = (sqrtf(1.17722499F - (pi_d_AxleOutput * pi_d_AxleOutput)) * 0.1F) +
      (0.9F * pmsmfoc_rtDWork.UnitDelay5_DSTATE);
   ...
   *  Outport: '<Root>/KpCoef'
   */
  currentloop(&pmsmfoc_rtU.Drive, &PMC_IsdDes, &PMC_isdFlt, &PMC_IsqDes,
    ...
    real32_T *)&f32_P_Modulation, &PMC_PsiExc, (const real32_T *)
              ...
              &(pmsmfoc_rtDWork.Model1_InstanceData.rtdw));
   ...
   */
  SVPWM(&pmsmfoc_rtU.Drive, &rtb_Gain3_f, &rtb_Sqrt_l, &rtb_angleElec, &rtb_Add3,
        &pmsmfoc_rtY.f32_Duty, &cmprU, &cmprV, &cmprW,
        &(pmsmfoc_rtDWork.Model2_InstanceData.rtdw));
...
void FOC_initialize(void)
{
  ...
  idkqe0cpzy_prot(&(pmsmfoc_rtDWork.Model1_InstanceData.rtdw));
}
