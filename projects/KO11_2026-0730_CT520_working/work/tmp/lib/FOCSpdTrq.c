...
void FOCSpdTrq_step(void)
{
   ...
   *  Inport: '<Root>/TrqMax'
   */
  rtb_Abs1 = fminf(GRSU.TrqMax, g_msgCore0to2_maxTrq2);
   ...
   *  Inport: '<Root>/TrqMin'
   */
  rtb_Divide1 = fmaxf(-g_msgCore0to2_maxTrq2, GRSU.TrqMin);
  ...
  if ((GRSU.ControlMode == nCtrl) == ((int32_T)GRSU.Drive)) {
    ...
    sinfixtrq = sinf((g_msgCore0to2_sin_frq * rtb_Switch_d) +
                     g_msgCore0to2_sin_phase) * g_msgCore0to2_sin_Amp;
    ...
    cosfixtrq = cosf((g_msgCore0to2_cos_frq * rtb_Switch_d) +
                     g_msgCore0to2_cos_phase) * g_msgCore0to2_cos_Amp;
     ...
     */
    asrKiHi = (sqrtf(fmaxf(0.0F, (g_msgCore0to2_maxCur2 * g_msgCore0to2_maxCur2)
      - (GRSU.isdSet_last * GRSU.isdSet_last))) * 0.1F) +
      (GRSDWork.UnitDelay1_DSTATE_o * 0.9F);
     ...
     */
    if (g_msgCore0to2_CalibEn2) {
      rtb_Switch1 = fminf(fminf(fminf(f32_P_SpeedPiMaxRatio, 1.0F), fabsf
        (GRSU.SpeedPiMaxRatio)) * g_msgCore0to2_maxCur2, asrKiHi);
      rtb_Switch2 = -fminf(asrKiHi, g_msgCore0to2_maxCur2 * fminf(fminf(fabsf
    ...
    asr_rpm_Lo = fabsf(GRSU.f32_speedfinal);
    ...
    rtb_Switch1 = fabsf(err_abs - rtb_Switch2);
     ...
     *  MinMax: '<S2>/Max2'
     */
    Trqcomp_Max_P1 = fminf(rtb_Abs1, g_msgCore0to2_maxTrq2) - PIout;
     ...
     *  MinMax: '<S2>/Max3'
     */
    Trqcomp_Min_P1 = fmaxf(-g_msgCore0to2_maxTrq2, rtb_Divide1) - PIout;

    /* Lookup_n-D: '<S2>/Amplitude_Table' incorporates:
     *  Abs: '<S2>/Abs'
     *  Inport: '<Root>/f32_SpeedAct_rpm'
     */
    K1_value_P1 = look1_iflf_pbinlca(fabsf(GRSU.f32_SpeedAct_rpm), (const
      real32_T *)&cripple_T_P1.bp1[0], (const real32_T *)&cripple_T_P1.tab[0],
      &GRSDWork.m_bpIndex, 14U);

    /* Lookup_n-D: '<S2>/Amplitude_Table1' incorporates:
     *  Inport: '<Root>/f32_SpeedAct_rpm'
     */
    K3_value_P1 = look1_iflf_pbinlca(GRSU.f32_SpeedAct_rpm, (const real32_T *)
      &cphase_T_P1.bp1[0], (const real32_T *)&cphase_T_P1.tab[0],
      &GRSDWork.m_bpIndex_e, 32U);
     ...
     *  Switch: '<S2>/Switch6'
     */
    wave_P1 = sinf((g_msgCore0to2_K2 * rtb_Switch_d) + rtb_Abs1) * rtb_Divide1;
